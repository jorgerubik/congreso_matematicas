	...
	$conn = mysql_connect("$host", "$user", "$pass");
		...
		$query = "SELECT * FROM ponencias_curso WHERE RFC = '".$rfc."'"."AND id_ponencia_curso = '".$id_trabajo."'";
					...
					while ($row = mysql_fetch_assoc($r)){
						...
						echo"<form action='editar_curso_confirma.php' method='post' autocomplete='off' class='forms' id='formulario'>";
						echo"<fieldset id='ponencia'><legend>Formulario de registro</legend><legend>Título (máximo 15 palabras)</legend>";
						echo"<input type='text'  id='titulo_curso' value='".$row['titulo_curso']."' name='titulo_curso' onblur='wordCountf();' required>";
